...
```
/smelter:research <goal>                    # Standard research with user checkpoints
/smelter:research AUTO: <goal>              # Fully autonomous until complete
/smelter:research status                    # Check current research session status
/smelter:research resume                    # Resume interrupted research session
/smelter:research list                      # List all research sessions
/smelter:research report <session-id>       # Generate report for session
```

### Quick Examples

```
/smelter:research What are the performance characteristics of different sorting algorithms?
/smelter:research AUTO: Analyze authentication patterns in this codebase
/smelter:research How does the error handling work across the API layer?
```
...
Task(subagent_type="smelter:scientist-low", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="smelter:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="smelter:scientist-high", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="smelter:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/smelter:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/smelter:research AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="smelter:scientist-low", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="smelter:scientist-low", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="smelter:scientist-low", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="smelter:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="smelter:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="smelter:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="smelter:scientist-high", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/smelter:research status` | Show current session progress |
| `/smelter:research resume` | Resume most recent interrupted session |
| `/smelter:research resume <session-id>` | Resume specific session |
| `/smelter:research list` | List all sessions with status |
| `/smelter:research report <session-id>` | Generate/regenerate report |
| `/smelter:research cancel` | Cancel current session (preserves state) |
